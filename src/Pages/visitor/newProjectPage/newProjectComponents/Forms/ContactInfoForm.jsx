  ...
  return (
    <>
      <div className='contact_info_form'>
        ...
        >
          {t('Hello_my_friend')}<HiIcon/><SmileIcon/>
        </motion.h3>
        <motion.p
         className='form_1_p'
         initial={init}
        ...
        >
          {t('for_1_p')}<a href='#'><MessageIcon/>{t('my_email')}</a>
        </motion.p>
        <motion.div
          initial={init}
          animate={animation}
          transition={{type: 'spring', bounce: .3, duration: 1, delay: .1777777}}
        ><HandIcon/>
        </motion.div>
      </div>
      <div className="input_control">
        ...
        initial={init}
        animate={animation}
        transition={{type: 'spring', bounce: .3, duration: 1, delay: .25}}
        className="flex"
        >
          <UserNameIcon/>{t('input_title_one')}
        </motion.h2>
        <motion.div 
          initial={init}
          animate={animation}
          transition={{type: 'spring', bounce: .3, duration: 1, delay: .35}}
      ...
      <div className="input_control">
        <motion.h2 
          initial={init}
          animate={animation}
          transition={{type: 'spring', bounce: .3, duration: 1, delay: .45}}
          className="flex"
        >
          <MessageIcon/>{t('input_title_2')}
        </motion.h2>
        <motion.div 
          initial={init}
          animate={animation}
          transition={{type: 'spring', bounce: .3, duration: 1, delay: .55}}
          className="position-relative"
        >
          <Input 
            input={{value: props.email}} 
            type='email' onId='email' 
          ...
          <div className="input_icons">
            ...
            {props.showMic === 'show' &&  <MIC onREC={props.onStartRecInputEmail} recording={props.InputEmailRecording} />} 
          </div>
          {props.onInputEmailError && <p className='error_input_prg flex'><ExitInvalidInput/>This field is not valid</p>}
        </motion.div>
      </div>
      <motion.div 
        initial={init}
        animate={animation}
        transition={{type: 'spring', bounce: 3, duration: 1, bounce: .3, delay: .65}}
        className="form_btns flex"
      >
        <button onClick={() => window.history.back()} className='btn_back' type='button'>{t('Back')}<ArrowUpIcon/></button>
        <Button 
          onClick={() => {
