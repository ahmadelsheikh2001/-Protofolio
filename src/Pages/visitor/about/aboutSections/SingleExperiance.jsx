    ...
    <div className="col-12 col-lg-6">
      <ScrollTransition elementTop={props.elementTop} delay={`.${props.index}`} classes="card side_border">
        ...
        <div className="comp-img">
          <img src={apiUrl + props?.logo} className='img-fluid' width="100px" />
        </div>
        <div className="comp-data">
          <h4>{ctx.lang === 'en' ?props?.job : props?.job_ar} <span>{ctx.lang === 'en' ? 'at' : 'فى'}</span> <a href={ctx.lang === 'en' ? props?.link : props?.link_ar} target='_blank'>{ctx.lang === 'en' ? props?.company : props?.company_ar}</a></h4>
          <p className="d-flex">{icon} {props?.start} / {props?.end}</p>
        </div>
      </ScrollTransition>
    </div>
  );
