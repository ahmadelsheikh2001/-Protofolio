...
const FeedbacksTable = () => {
  ...
  let filterdData = feeedbacks
  if (search) filterdData = filterdData.filter((ele) => ele?.name?.includes(search) || ele?.message?.includes(search))
  if (status) filterdData = filterdData.filter((ele) => ele.status == status)
  if(date) filterdData =filterdData.filter((ele)=>ele.date == date)
  ...
  return (
    <>
      <div className="table_filter">
        ...
        <div className="cases_input">
          ...
          <select value={status} onChange={(e) => setStatus(e.target.value)}>
            <option value="">كل التعليقات</option>
            <option value="approved">تم قبوله</option>
      ...
      <Table tableHead={tableHeaders}>
        {filterdData.map((feedback, index) => <SingleFeedback key={feedback.id} index={+index} feedback={feedback} />)}
