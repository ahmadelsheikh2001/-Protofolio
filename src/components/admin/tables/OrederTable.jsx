...
const OrederTable = () => {
  ...
  let filteredOrdered = data
  if (search) filteredOrdered = filteredOrdered.filter((ele) => ele?.name?.includes(search) || ele?.email?.includes(search))
  if (cases) filteredOrdered = filteredOrdered.filter((ele) => ele?.status == cases)
    ...
    <>
      <div className="table_filter">
        ...
        <div className="cases_input">
          {icons.caseStudy}
          <select value={cases} onChange={(e) => setCases(e.target.value)}>
            <option value="">all cases</option>
            <option value="pending">في الانتظار</option>
            <option value="approved">تمت الموافقة</option>
            <option value="rejected">تجاهلتة</option>
          </select>

        </div>
      </div>
      <Table tableHead={tableHeaders}>
        {filteredOrdered.map((order, index) => (
